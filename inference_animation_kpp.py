...
def export_animated_glb(initial_mesh, part_mask_np, qr_seq_tensor, qd_seq_tensor, 
    ...
    # 3. Setting the Node Coordinate System (Pivot Adjustment)
    # Key Step: To make the GLTF animation rotate around the joint axis, we need to:
    # A. Move the vertices of the Moving Mesh to a position with (0,0,0) as the joint center (Vertex - Origin)
    # B. Move the Nodes supporting the Mesh back to the joint's position in world coordinates (Node Position = Origin)
    ...
    # A. (DualQuaternion Real Part -> GLTF Quaternion)
...
def load_kpp_model(checkpoint_path: str, device: torch.device) -> Optional[KeypointPredictor]:
    ...
    try:
        ...
        print(f"KPP strict load failed: {exc}")
...
def run_vae_diversity_test(model, loader, sample_idx, device, output_dir, num_frames,
                           num_samples_to_gen, force_rotation=False,
                           kpp_model: Optional[KeypointPredictor] = None):
    """
...
def parse_args():
    ...
    parser.add_argument('--kpp_checkpoint', type=str, default=None,
                    help='Path to KPP weights (optional)')
