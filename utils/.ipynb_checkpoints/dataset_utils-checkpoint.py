...
class KPPDataset(FixedDragMeshDatasetV2):
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        # 7. (关键修复!) QD (平移) 现在依赖于 joint_type
        if joint_type == 0:
            # 旋转 (Revolute):
            # 因为我们已将 'origin' 移到 (0,0,0)，
            # 绕原点旋转的 'qd' Gound Truth 永远是 0。
            qd_gt = np.zeros_like(qr_gt)
        else:
            # 平移 (Prismatic):
            # 平移的 qd = 0.5 * [0, t_x, t_y, t_z]
            # t_vec = axis * distance (angle)
            # 我们只需要缩放这个 t_vec, 也就是缩放 qd。
